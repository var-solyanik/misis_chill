...
class Raindrop():
	...
	def updateTexture(self, bg): 	
		fg = GaussianBlur(Image.fromarray(np.uint8(bg)), random.uniform(6., 15.))
		fg = np.asarray(fg)
	...
	def _createDefaultDrop(self):
		...
		if (self.shape == 0):    
			...
			self.alphamap = GaussianBlur(Image.fromarray(np.uint8(self.labelmap)), self.blur_coeff)  
		...
		if (self.shape == 2): 
			...
			self.alphamap = GaussianBlur(img, self.blur_coeff)       
			self.alphamap = np.asarray(self.alphamap).astype(float)
			self.alphamap = self.alphamap/np.max(self.alphamap)*255.0
			# set label map
			self.labelmap[self.labelmap>0] = 1            
